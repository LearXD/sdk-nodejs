...
import type { PaymentSearchData } from './search/types';
...
export class Payment {
	 ...
	 */
	search(paymentSearchOptions: PaymentSearchData = {}): Promise<PaymentResponseType> {
	 ...
	 */
	cancel({ id, requestOptions }: PaymentCancelData): Promise<PaymentResponseType> {
		this.config.options = { ...this.config.options, ...requestOptions };
	 ...
	 */
	capture({ id, transaction_amount, requestOptions }: PaymentCaptureData): Promise<PaymentResponseType> {
		this.config.options = { ...this.config.options, ...requestOptions };
	 ...
	 */
	create({ body, requestOptions }: PaymentCreateData): Promise<PaymentResponseType> {
		this.config.options = { ...this.config.options, ...requestOptions };
	 ...
	 */
	get({ id, requestOptions }: PaymentGetData): Promise<PaymentResponseType> {
		this.config.options = { ...this.config.options, ...requestOptions };
